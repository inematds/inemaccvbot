...
export class StateStore {
  ...
  track(j: Omit<TrackedJob, 'lastStatus' | 'createdAt' | 'narracaoPath' | 'transcrever'>
    ...
    this.db.prepare(`INSERT INTO tracked_jobs (queue, job_id, chat_id, dest, dest_token, pesquisa, transcrever, narracao_path)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ...
      .run(j.queue, j.jobId, j.chatId, j.dest, j.destToken, j.pesquisa ? 1 : 0, j.transcrever ? 1 : 0, j.narracaoPath ?? null);
  }
